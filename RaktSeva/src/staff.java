...
public class staff extends JFrame implements ActionListener
{
	...
	JComboBox JCB1,JCB2;
	staff()
	{ setContentPane(new JLabel((new ImageIcon(("C:\\Users\\star\\Downloads\\img_15.jpeg.jpg")))));
		...
		JB4=new JButton("Manage reciever");
		JB5=new JButton("Reset Password");
		JB6=new JButton("Provide Blood");
		JB7=new JButton("Logout");
		JB7.setFont(new Font("Trebuchet MS",Font.BOLD,18));
		JB7.setBounds(300,450,130,50);
		add(JB7);

		setLayout(null);
		JL1.setBounds(200,90,430,50);
		JL1.setFont(new Font("Trebuchet MS",Font.BOLD,32));
...
//
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(e.getSource()==JB5)
		{
			...
			String q="select*from user where UserID="+"'"+ID+"'"+"AND "+"password="+"'"+Password+"'";
			ResultSet r=null;
			try {
				...
				if(r.next())
				{
					...
					String q2="UPDATE user SET password="+"'"+newpass+"'"+"where UserID="+"'"+ID+"'";
		...
		else if(e.getSource()==JB4)
		{
			...
			if(JCB2.getSelectedItem().equals("Update"))
			{
				...
				if(JCB3.getSelectedItem().equals("Update Name"))
				{
					...
					String s=JOptionPane.showInputDialog(JT1,"Enter Patient ID");
					String Query="select*from reciever where PatientID='"+s+"'";
					try
					{
						...
						if(r.next())
						{
							...
							String s2=JOptionPane.showInputDialog(JT1,"Enter Patient Updated Name");
							String Q="UPDATE reciever SET PatientName='"+s2+"' Where PatientID='"+s+"'";
							c.s.executeUpdate(Q);
							JOptionPane.showMessageDialog(null,"Name Updated Successfully\nThank You");
						}else
						{
							JOptionPane.showMessageDialog(null,"Patient Not In\nOur Record");
						}
					}catch(Exception a)
					{
						a.getStackTrace();
				...
				}else if(JCB3.getSelectedItem().equals("Update phonenumber"))
				{
					...
					try
					{
						...
						{
							JTextField JT2=new JTextField(20);
							String s2=JOptionPane.showInputDialog(JT1,"Enter Patient\n Updated contactNumber");
							while(s2.length()!=10)
							{
								s2=JOptionPane.showInputDialog(JT1,"Entered Incorrect contactNumber\nplease Enter Again");
							}
							String Q="UPDATE reciever SET contactNumber='"+s2+"' Where PatientID='"+s+"'";
							c.s.executeUpdate(Q);
			...
			{
				String x=JOptionPane.showInputDialog(JT1,"Enter Patient ID");
				String st="Select*from reciever where PatientID="+"'"+x+"'";
				try {
					ResultSet r=c.s.executeQuery(st);
					if(r.next())
					{
						String message="PatientID : "+r.getString("PatientID")+"\n"+"Patient Name: "+r.getString("PatientName")+"\n"+
								"PatientAge: "+r.getString("PatientAge")+"\n"+"contactNumber: "+r.getString("contactNumber");
			...
			{
				String x=JOptionPane.showInputDialog(JT1,"Enter Patient ContactNumber");
				String st="Select*from reciever where contactnumber="+"'"+x+"'";
				try {
					ResultSet r=c.s.executeQuery(st);
					if(r.next())
					{
